...
class DeserializationStrategyFactory:
    ...
    def get_strategy(self, *, format: Optional[SupportedFormat] = None, format_data_source: Optional[DataSource], **kwargs) -> Optional["DeserializationStrategy"]:
        ...
        if strategy_class is None:
            registered_strategies = list(filter(lambda strategy: strategy.can_deserialize_format(format, format_data_source), self._registered_strategies.values()))
...
@inject
def register_deserialization_strategy(strategy: type["DeserializationStrategy"], factory: DeserializationStrategyFactory) -> type["DeserializationStrategy"]:
